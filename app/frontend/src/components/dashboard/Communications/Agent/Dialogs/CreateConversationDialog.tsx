  ...
  async function createConversation(partner: Business) {
    ...
    try {
      ...
      const addParticipantPromises = [
        addParticipant(agent.user.id, "agent", conversationId),
        addParticipant(partner.user.id, "business", conversationId),
      ];

      await Promise.all(addParticipantPromises);

      successToastFactory(toast, "Conversation was created successfully.")
      setOpen(!open);
      ...
      {isLoading ? (
        ...
        <div>
          {partners.map(partner => (
            <div onClick={() => createConversation(partner)} key={partner.id} className="flex justify-between items-center h-14 border-b hover:scale-[1.01] cursor-pointer duration-300">
              <div className="flex items-center gap-x-3">
                <Avatar>
                  ...
                  />
                  <AvatarFallback>
                    {partner.profile.name.split(" ").map(str => str.charAt(0))}
                  </AvatarFallback>
                </Avatar>
