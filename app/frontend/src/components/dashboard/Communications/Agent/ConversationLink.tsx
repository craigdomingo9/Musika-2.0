...
function ConversationLink({partner}: Props) {
  ...
  return (
    <div className="flex justify-between items-center h-20 border-b hover:scale-[1.01] cursor-pointer duration-300">
      <div className="flex items-center gap-x-3">
        <Avatar>
          <AvatarImage 
            src={partner?.user.business_profile?.profile.logo} 
            alt={`Logo for ${partner.user.business_profile?.profile.name}`} 
          />
          <AvatarFallback>
            {initials}
          </AvatarFallback>
        ...
        <div className="font-semibold text-opacity-mid text-sm">
          {partner.user.business_profile?.profile.name}
        </div>
      </div>
