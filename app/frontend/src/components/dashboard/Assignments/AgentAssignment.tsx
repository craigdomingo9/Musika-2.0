...
export const useAssignmentDrawerState = createEntityStore(false);
export const useAssignmentMutation = createEntityStore(createEntityAction<Assignment>())
export const useAssignmentAction = createEntityStore(false);


function AgentAssignment({assignment}: Props) {
  const { entities: open, setEntities: setOpen } = useAssignmentDrawerState();
  const { setEntities: setAssignmentMutation } = useAssignmentMutation();
  ...
  return (
    <>
      ...
      <DrawerContainer 
        open={open}
        setOpen={setOpen}
        Content={<AssignmentDrawerContent />}
      />
    </>
  )
}
